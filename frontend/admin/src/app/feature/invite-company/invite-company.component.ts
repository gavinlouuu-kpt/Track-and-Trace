...
import { COMPANY_ADDRESS, INVITE } from '../company/company.config';
import { IMPORTS } from './invite-company.config';
...
export class InviteCompanyComponent implements OnInit, OnDestroy {
  ...
  addMore(): void {
    const supplyChainIdsToRemove = this.addedSupplyChains.map(
      (item: any) => item.id
    );
    this.supplyChainList = this.supplyChainList.filter(
      (item: any) => !supplyChainIdsToRemove.includes(item.id)
    );
  ...
  removeField(index: number): void {
    const removedSupplyChainId = this.supplyChains
      .at(index)
      .get('supplyChain').value;
    if (removedSupplyChainId) {
      const removedItem = this.addedSupplyChains.find(
        item => item.id === removedSupplyChainId
      );

      if (removedItem) {
        this.supplyChainList = [...this.supplyChainList, removedItem];
        this.addedSupplyChains = this.addedSupplyChains.filter(
          item => item.id !== removedSupplyChainId
        );
      }
    }
    ...
    if (this.supplyChains.controls.length > 1) {
      this.supplyChainList = [
        ...this.supplyChainList,
        this.addedSupplyChains[0],
      ];
    }
  }
  ...
  dropdownFormValue(value: any, label: string, index: number): void {
    ...
    if (label === 'type') {
      ...
      if (selected && value.id !== 'All') {
        this.addedSupplyChains = this.supplyChains.value
          .filter((item: any) => item.supplyChain)
          .map((item: any) => {
            return (
              this.supplyChainList.find(
                (option: any) => option.id === item.supplyChain
              ) ||
              this.addedSupplyChains.find(
                (option: any) => option.id === item.supplyChain
              )
            );
          });
  ...
  dropDownValueAddres(value: any, label: string): void {
    ...
    if (label === 'country') {
      ...
      this.addressForm.get('inDialCode').patchValue(`+${value?.dial_code}`);
      if (selected) {
  ...
  /**
   * The function `companyNameCheck` checks if a company name already exists in the database and sets
   * an error if it does.
   */
  companyNameCheck(): void {
    const nameFormControl = this.inviteForm.get('name');
